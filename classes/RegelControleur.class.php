...
class RegelControleur
{
    ...
    public function bevatSteenVanTegenstander($positie, $bord, $spelerAanDeBeurt)
    {
        if ($this->bevatSteen($positie, $bord)) {
            if ($bord->vakjes[$positie->y][$positie->x]->steen->kleur === $this->kleurVanSpeler(1 - $spelerAanDeBeurt->id)) {
                $this->over = $positie;
                return true;
            }
        }
    ...
    private function vakkenVanBeschikbareStenen($bord, $spelerAanDeBeurt)
    {
        ...
        foreach ($bord->vakjes as $rijNummer => $rij) {
            foreach ($rij as $kolomNummer => $vak) {
                if (isset($vak->steen)) {
                    if ($vak->steen->kleur === $spelerAanDeBeurt->kleur) {
